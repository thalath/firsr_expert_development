...
# ========================================================
# JSON Schema(loaded from external file)
# ========================================================
...
def _load_schemas_from_file(path: str):
    ...
    spec.loader.exec_module(module) # type: ignore[attr-definded]
    try:
...
def validate_payload():
    if request.method == "GET":
        return jsonify({
            "usage": "POST JSON with 'facts' and/or 'rules' to available.",
    ...
    if "rules" in payload:
        try:
            validate(payload["rule"], rules_array_schema)
        except ValidationError as e:
...
def infer():
    if request.method == "GET":
        ...
        weights = {}
        if weights_param:
            for pair in weights_param.splite(","):
                if ":" in pair:
                    fid, conf = pair.splite(":", 1)
                    try:
                        weights[fid.strip()] = float(conf.strip(conf.strip()))
                    except ValueError:
                        pass
        else:
            payload = request.get_json(force=True, silent=True) or {}
        ...
        for fid in observed_ids:
            obs_conf[fid] = clamp01(weights.get(fid, 2.0))
...
def facts_new():
    if request.method == "POST":
        ...
        value = (request.form.get("value") or "").strip() == "on"
        tags_row = request.form.get("tags") or ""
        tags = [t.strip() for t in tags_row.split(",") if t.strip()]

        if not fid:
            flash("ID is required", "danger")
            return render_template("facts_form.html", mode="new", fact=None)
        if any(t.get("id") == fid for t in facts ):
...
def facts_edit(fid):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            flash(f"Validation error: {e.message}", "danger")
            return render_template("facts_form.html", mode="edit", fact=updated)

        facts.clear(); facts.extend(new_facts)
        global FACT_VALUE
        FACT_VALUE = {f["id"]: bool(f.get("value", False)) for f in facts}
        flash(f"Fact updated", "success")
        return redirect(url_for("facts_list"))

    return render_template("facts_form.html", mode="edit", fact=fact)
...
def rules_new():
    if request.method == "POST":
        ...
        if not rid:
            flash("ID is required", "danger")
            return render_template("rules_form.html", mode="new", rule=None)
        ...
        try:
            ...
            flash(f"Validation error: {e.message}", "danger")
            return render_template("rules_form.html", mode="new", rule=new_item)
...
def rules_edit(rid):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            flash(f"Validation error: {e.message}", "danger")
            return render_template("rules_form.html", mode="new", rule=updated)
        
        rules.clear(); rules.extend(new_rules)
        flash("Rule updated", "success")
        return redirect(url_for("rules_list"))
        
    return render_template("rules_form.html", mode="edit", rule=rule)
...
def taxonomy_save():
    ...
    try:
        ...
        return render_template("taxonomy.html", taxonomy=taxonomy, raw_overried=raw)
